...
void switch_m2u_mode() {
  ...
  asm volatile ("la x6, move_on_mark":::);
  asm volatile ("csrw mepc, x6":::);
  asm volatile ("mret":::);
  asm volatile ("move_on_mark:":::);
} 

uint32_t N200_getMtimeLo(void) {
  return *(volatile uint32_t *)(0xD1000000);
}

uint32_t N200_getMtimeHi(void) {
  return *(volatile uint32_t *)(0xD1000004);
}

uint64_t N200_getTimerValue() {
  while (1) {
